...
# ========== Config ==========
load_dotenv()
PDF_PATH = os.getenv("PDF_PATH", "data/AI.pdf")
...
def _approx_tokens_from_chars(s: str) -> int:
    return max(1, math.ceil(len(s) / 4))  # Rough 4 chars/token heuristic
...
def _build_index(pdf_path: str, model: SentenceTransformer):
    ...
    embs = model.encode(passages, batch_size=32, show_progress_bar=False,
...
def ask_get(
    question: str = Query(..., description="Your question via query param"),
...
def ask_post(body: AskBody):
    try:
        ...
        k = body.top_k or TOP_K_DEFAULT
        scores, idxs = _search(_index, _model, question, k=k)
        hits = []
        for score, idx in zip(scores.tolist(), idxs.tolist()):
            if idx == -1:
                continue
